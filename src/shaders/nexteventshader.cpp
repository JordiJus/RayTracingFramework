#include "nexteventshader.h"
#include "../core/utils.h"
#include "../core/hemisphericalsampler.h"

NextEventShader::NextEventShader() :
    Shader(Vector3D(0.0))
{}

NextEventShader::NextEventShader(double _numSamples, double _maximumDepth) :
    Shader(Vector3D(0.0)), numSamples(_numSamples), maximumDepth(_maximumDepth)
{}

Vector3D NextEventShader::computeColor(const Ray& r, const std::vector<Shape*>& objList, const std::vector<LightSource*>& lsList) const
{
    Intersection its;
    double Vx = 1;
...
Vector3D NextEventShader::reflectedRadiance(const Intersection its, const Ray r, const std::vector<Shape*>& objList, const std::vector<LightSource*>& lsList, int maxDepth) const
{
    Vector3D Ldir(0.0);
    Vector3D Lindir(0.0);
...
Vector3D NextEventShader::directRadiance(const Intersection its, const Ray r, const std::vector<Shape*>& objList, const std::vector<LightSource*>& lsList) const
{
...
Vector3D NextEventShader::indirectRadiance(const Intersection its, const Ray r, const std::vector<Shape*>& objList, const std::vector<LightSource*>& lsList, int maxDepth) const
{
    Intersection its2;
